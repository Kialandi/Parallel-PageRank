...
int main(int argc, char* argv[]){
    ...
    double ckpt1, ckpt2, ckpt3, ckpt4, ckpt5;
    struct timeval tz;
    struct timezone tx;
    ...
    if (world_rank == 0) {
        ...
        /*
           if (leftover > 0) {
           leftoverDestOf = (int **) malloc(leftover * sizeof(int *));
           for (i = 0; i < leftover; i++) {
//worst case need to send to all procs
leftoverDestOf[i] = (int *) malloc(world_size * sizeof(int));
int j;
for (j = 0; j < world_size; j++) {
        //init to all -1
        leftoverDestOf[i][j] = -1;
        }
        }    
        }      */ 
        ...
        if (leftover > 0) {
            ...
            // printf("starting from index %d of colind, calculating %d ele\n", colArrIndex, leftoverEles);
    ...
    mapping = (double **) malloc(n * sizeof(double *));
    ...
    for (i = 0; i < n; i++) {
        ...
        if (expectingFrom[i] == 0) {
            mapping[i] = NULL;
        }
        else if (owner == world_rank) {
            //yourself
            //printf("yourself\n");
            mapping[i] = &vector[i];//somewhere
            //printf("%d val mapped to %lf\n",i, *mapping[i]);

        }
        else {
            //assume u have made a recvFrom entry already
            int j;
            for (j = 0; j < recvFromArr[owner]->numOfElements; j++) {
                //printf("entry %d from %d\n",j,  owner);
                mapping[i++] = &(recvFromArr[owner]->recvBuf[j]);
                //printf("%d val mapped to %lf\n",i-1, *mapping[i -1]);
            }
            i--;
        }
    ...
    while(1) {
        ...
        if (world_rank == 0) {
            printf("multiplying by\n");
            printVecMapping(n, mapping);
        }

        //csrMatVec(rowPerProc, alpha, value, colind, rbegin, vector, &(result[location])); 
        csrMatVec(rowPerProc, alpha, value, colind, rbegin, mapping, &(result[location])); 
        if (leftover > 0 && world_rank == 0) {
            //matvec for leftover array at p0
            csrMatVec(leftover, alpha, leftoverVal, leftoverColind, leftoverRbegin, mapping, &result[leftoverLoc]); 
        }
        ...
        epsi = computeChange(rowPerProc, &result[location], mapping, location);

        if (leftover > 0 && world_rank == 0) {
            double leftoverEpsi = computeChange(leftover, &result[leftoverLoc], mapping, leftoverLoc);
        ...
        if (world_rank == 0) {
            printf("result arr before group comm\n");
            printVecMapping(n, mapping);
        }
        /*******************************************************************************************
         *
        ...
        if (world_rank == 0) {
            //stop timer
            gettimeofday(&tz, &tx);
            ckpt1 = (double)tz.tv_sec + (double) tz.tv_usec / 1000000.0;
        }
        ...
        if (world_rank == 0) {
            //stop timer
            gettimeofday(&tz, &tx);
            ckpt2 = (double)tz.tv_sec + (double) tz.tv_usec / 1000000.0;
        }
        /*******************************************************************************************
         *
         *                              Receiving loop
        ...
        if (world_rank == 0) {
            //stop timer
            gettimeofday(&tz, &tx);
            ckpt3 = (double)tz.tv_sec + (double) tz.tv_usec / 1000000.0;
        }
        //if (iterations == 1) {
            //rewrite mappings
            for (i = 0; i < rowPerProc; i++) {
                mapping[location] = &result[location];
            }
        //} 
        
        MPI_Barrier(MPI_COMM_WORLD);
        if (world_rank == 0) {
            printf("result arr end loop\n");
            printVecMapping(n, mapping);
        }
